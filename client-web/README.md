...
> The FastAPI server enables CORS for allowed origins, with http://localhost:8001 included by default. If you host the client elsewhere, set `ALLOWED_ORIGINS` (comma-separated) in your `.env` or `config/settings.yaml` so the browser can reach the API.
